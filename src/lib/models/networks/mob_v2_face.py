...
def mobilenet_v2(
    *, heads, head_conv, face_heads, weights: Optional[MobileNet_V2_Weights] = None, progress: bool = True, **kwargs: Any
) -> MobileNetV2:

    # if weights is not None:
    #     _ovewrite_named_param(kwargs, "num_classes", len(weights.meta["categories"]))

    model = MobileNetV2(heads, head_conv, face_heads , width_mult = 1.0, **kwargs)
...
def get_pose_net_mob_face(num_layers,heads,head_conv):



    face_heads = {'face_hm': 1}
    model = mobilenet_v2(heads = heads, face_heads = face_heads, head_conv=head_conv)
