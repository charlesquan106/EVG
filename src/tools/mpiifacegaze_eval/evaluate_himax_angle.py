...
from mpiifacegaze_eval_lib.utils.common_himax import angular_error, get_target_on_screen, vector_to_angle, angular_error_no_pitchyaw_to_vector
import cv2
...
def test(model, test_loader, opt):
    ...
    monitor_mm = (295,166)
    monitor_pixels = (1366,768)
    screen_height_mm_offset = 78  # measure manually
    ...
    with torch.no_grad():
        for iter_id, batch in enumerate(test_loader):
            ...
            gaze_origin = batch['meta']['gaze_origin']
            ...
            dets_sc_org_coord = torch.tensor([[(dets_org_coord[0][0]-2417),(dets_org_coord[0][1]-1541)]], dtype=torch.float32)
                            

            
            # width  vp + (sc/2) - (vp/2) =  vp + (1366/2) - (6200/2) = vp - 2417
            # height  vp + (sc/2) - (vp/2) - camera_screen_y_offset(sc_height/2 + 78* vp_pixel_per_mm ) =  vp + (768/2) - (2360/2) - (768/2+78*4.63)  =  vp - 1541
            ...
            gaze_origin = batch['meta']['gaze_origin'][0].numpy()
            gaze_target = batch['meta']['gaze_target'][0].numpy()
            
            # print("gaze_origin: ",gaze_origin)
            # print("gaze_target: ",gaze_target)
            
            direction_gt = gaze_target - gaze_origin
            direction_gt_pitchyaw = vector_to_angle(direction_gt[0]) 
            ...
            dets_gt_org_coord_mm = get_target_on_screen(dets_sc_org_coord[0], monitor_mm, monitor_pixels, screen_height_mm_offset,screen_depth_mm_offset)
            # dets_gt_org_coord_mm = 
            dets_gt_org_coord_mm = dets_gt_org_coord_mm.reshape(1,3)
            
            # print("dets_gt_org_coord_mm: ",dets_gt_org_coord_mm)
            # print("dets_gt_org_coord_mm: ",type(dets_gt_org_coord_mm))
            
            # print("batch['meta']['gaze_origin']: ",type(batch['meta']['gaze_origin'].numpy()))
            
            # print("batch['meta']['gaze_origin']: ",batch['meta']['gaze_origin'])
            # print("batch['meta']['gaze_origin']: ",batch['meta']['gaze_origin'].shape)
            
            direction =  dets_gt_org_coord_mm - gaze_origin
            ...
            direction_pitchyaw = vector_to_angle(direction[0]) 
            ...
            direction_error = abs(direction_gt_pitchyaw - direction_pitchyaw)
            ...
            angle_error = angular_error_no_pitchyaw_to_vector(direction_gt,direction)
