...
class Cross_EVE_Himax(data.Dataset):
  num_classes = 1
  ...
  def __init__(self, opt, split):
    super(Cross_EVE_Himax, self).__init__()
    _data_name = {'train': f'gaze_EVE_ffmpeg_ld', 'val': f'gaze_Himax'}
    self.data_dir = os.path.join(
      opt.data_dir,'{}').format(_data_name[split])
    
    # self.data_dir = os.path.join(opt.data_dir, 'gaze_EVE_ffmpeg_ld')
    # self.img_dir = os.path.join(self.data_dir, 'images')
    _ann_name = {'train': f'train', 'val': f'Himax_all_test'}
    self.annot_path = os.path.join(
      self.data_dir, 'annotations', 
      'gaze_{}.json').format(_ann_name[split])
    
    
    _images_name = {'train': f'EVE', 'val': f'Himax_all_test'}
