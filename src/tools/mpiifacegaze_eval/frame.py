...
def main(opt):
    ...
    mean = np.array([0.485, 0.456, 0.406],
                   dtype=np.float32).reshape(1, 1, 3)
    std  = np.array([0.529, 0.224, 0.225],
                   dtype=np.float32).reshape(1, 1, 3)
    ...
    folder_path = '/home/owenserver/Python/Test/data_normalization_code/image_train_10/'
    ...
    PoG = np.array([[957.7641 , 544.72797],
            [957.2596  ,545.1566 ],
            [957.49084 ,543.84827],
            [956.5849  ,541.91895],
            [957.85583 ,543.0358 ],
    ...
    # PoG = np.array([[ 963.66907 , 543.49335],
    #         [ 964.5139  , 543.01215],
    #         [ 961.47906 , 545.93756],
    #         [ 961.89545 , 543.7163 ],
    #         [ 961.47186 , 543.2818 ],
    ...
    for idx, img_path in enumerate(image_files):
        ...
        if img is not None:
            img_list.append((img, img_path , PoG[idx]))
        else:
            print(f"無法讀取圖像：{img_path}")
    
    for img, img_path, PoG  in img_list:
        print(img_path)
        ...
        # dets_modified = (int((dets_org_coord[0][0] + (sc_width/2) - (vp_width/2))),int((dets_org_coord[0][1]-(vp_height/2))))
