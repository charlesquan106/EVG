...
def get_anno_info( anno_dir: pathlib.Path) -> pd.DataFrame:
    ...
    select_names = [
                    ...
                    'vector_error_deg_pitch','vector_error_deg_yaw',]
...
def main():
    ...
    datadir_1 = '/home/master_111/nm6114091/Python/test/violin_chart/GazeCapture/GazeCapture_data/GazeCapture_anno_gaze_test.txt'
    datadir_2 = '/home/master_111/nm6114091/Python/test/violin_chart/GazeCapture/GazeCapture_data/GazeCapture_anno_gaze_test_phone.txt'
    datadir_3 = '/home/master_111/nm6114091/Python/test/violin_chart/GazeCapture/GazeCapture_data/GazeCapture_anno_gaze_test_tablet.txt'
    ...
    bins = np.linspace(-60, 60, 60)  # 40個bins，範圍是-100到100
    width = np.diff(bins)[0] / 1.0  # 每個條形的寬度，設置為bin寬度的一半
    ...
    plt.bar(bins1[:-1], hist1_percent, width=width/3, alpha=0.5, label='All', align='edge', edgecolor='black',color='blue')

    # 繪製第二組數據的直方圖，右移width / 2
    plt.bar(bins2[:-1] + width / 3, hist2_percent, width=width/3, alpha=0.5, label='Phone', align='edge', edgecolor='black',color='#FF5809')
        
    # 繪製第二組數據的直方圖，右移width / 2
    plt.bar(bins3[:-1] - width / 3 , hist3_percent, width=width/3, alpha=0.5, label='Tablet', align='edge', edgecolor='black',color='green')
    ...
    data_x_1 = combined_df_1['normalized_vector_angle_yaw'].values
    data_y_1 = combined_df_1['vector_error_deg_yaw'].values
    
    data_x_2 = combined_df_2['normalized_vector_angle_yaw'].values
    data_y_2 = combined_df_2['vector_error_deg_yaw'].values
    
    data_x_3 = combined_df_3['normalized_vector_angle_yaw'].values
    data_y_3 = combined_df_3['vector_error_deg_yaw'].values
    
    


    datasets = [
        (data_x_1, data_y_1, 'o', '#1f77b4', 'All'),        # 全圈
        (data_x_2, data_y_2, '^', '#ff7f0e', 'Phone'),      # 三角形
        (data_x_3, data_y_3, 'x', '#2ca02c', 'Tablet')      # 打叉
    ]


    bin_width = 2
    bins = np.arange(-60, 60 + bin_width, bin_width)  # 設定 x 軸區間上下限
    bin_centers = (bins[:-1] + bins[1:]) / 2

    plt.figure(figsize=(10, 6))

    for data_x, data_y, marker, color, label in datasets:
        # 計算每個區間內的平均 y 值，忽略空區間
        digitized = np.digitize(data_x, bins)
        bin_means = []
        for i in range(1, len(bins)):
            values_in_bin = data_y[digitized == i]
        ...
        # 移除包含 NaN 的區間
        valid_bin_centers = bin_centers[~np.isnan(bin_means)]
        valid_bin_means = np.array(bin_means)[~np.isnan(bin_means)]

        # 繪製散點圖和連接線
        plt.scatter(valid_bin_centers, valid_bin_means, marker=marker, s=50, color=color, label=f'Mean error per bin - {label}')
        plt.plot(valid_bin_centers, valid_bin_means, linestyle='-', color=color, label=f'Line - {label}')

        # # 擬合二次多項式曲線
        # z = np.polyfit(valid_bin_centers, valid_bin_means, 2)
        # p = np.poly1d(z)
        # plt.plot(valid_bin_centers, p(valid_bin_centers), color=color, label=f'Quadratic fit - {label}')

    # 設置圖表標題和標籤
    plt.xlabel('Yaw (deg)')
    plt.ylabel('Mean error (deg)')
    plt.xlim(-60, 60)  # 設置 x 軸的上下限
    plt.legend()
    ...
    data_x_1 = combined_df_1['normalized_h_angle_yaw'].values
    data_y_1 = combined_df_1['vector_error_deg_yaw'].values

    data_x_2 = combined_df_2['normalized_h_angle_yaw'].values
    data_y_2 = combined_df_2['vector_error_deg_yaw'].values

    data_x_3 = combined_df_3['normalized_h_angle_yaw'].values
    data_y_3 = combined_df_3['vector_error_deg_yaw'].values
    ...
    ############ normalized_vector_angle_yaw ############
    # 示例数据

    data1 = combined_df_1['normalized_vector_angle_yaw']
    data2 = combined_df_2['normalized_vector_angle_yaw']
    data3 = combined_df_3['normalized_vector_angle_yaw']

    bins = np.linspace(-60, 60, 60)  # 40个bins，范围是-80到80
    width = np.diff(bins)[0] / 1.0  # 每个条形的宽度，设置为bin宽度的一半
    ...
    ax1.set_title('Horizontal gaze direction distribution')
    ax1.legend(loc='upper left')

    # 创建次轴
    ax2 = ax1.twinx()

    data_x_1 = combined_df_1['normalized_vector_angle_yaw']
    data_y_1 = combined_df_1['vector_error_deg_yaw']

    data_x_2 = combined_df_2['normalized_vector_angle_yaw']
    data_y_2 = combined_df_2['vector_error_deg_yaw']

    data_x_3 = combined_df_3['normalized_vector_angle_yaw']
    data_y_3 = combined_df_3['vector_error_deg_yaw']
    ...
    bin_width = 2
    bins = np.arange(-60, 60 + bin_width, bin_width)  # 设置 x 轴区间上下限
    bin_centers = (bins[:-1] + bins[1:]) / 2

    for data_x, data_y, marker, color, label in datasets:
        # 计算每个区间内的平均 y 值，忽略空区间
        digitized = np.digitize(data_x, bins)
        bin_means = []
        for i in range(1, len(bins)):
            values_in_bin = data_y[digitized == i]
        ...
        # 绘制散点图和连接线
        ax2.scatter(valid_bin_centers, valid_bin_means, marker=marker, s=50, color=color, label=f'Mean error per bin - {label}')
        ax2.plot(valid_bin_centers, valid_bin_means, linestyle='-', color=color)

    # 设置次轴的标签
    ax2.set_ylabel('Mean error (deg)')
    ax2.set_ylim(0, 12)  # 设置 y 轴的显示范围（根据数据调整）
    ax2.invert_yaxis()
    ...
    ############ normalized_h_angle_yaw ############
    
    # 示例数据

    data1 = combined_df_1['normalized_h_angle_yaw']
    data2 = combined_df_2['normalized_h_angle_yaw']
    data3 = combined_df_3['normalized_h_angle_yaw']
    ...
    ax1.set_xlabel('Yaw (deg)')
    ax1.set_ylabel('Percentage of images')
    ax1.set_title('Horizontal head pose distribution')
    ax1.legend(loc='upper left')
    # plt.legend()
    ...
    data_x_1 = combined_df_1['normalized_h_angle_yaw']
    data_y_1 = combined_df_1['vector_error_deg_yaw']

    data_x_2 = combined_df_2['normalized_h_angle_yaw']
    data_y_2 = combined_df_2['vector_error_deg_yaw']

    data_x_3 = combined_df_3['normalized_h_angle_yaw']
    data_y_3 = combined_df_3['vector_error_deg_yaw']
