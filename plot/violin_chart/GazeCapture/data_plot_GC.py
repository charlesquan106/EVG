...
def main():
    ...
    datadir = os.path.join(os.getcwd(),"GazeCapture_data", args.datadir)
    data_df(datadir)
    
    
    
    # datadir = '/home/master_111/nm6114091/Python/test/violin_chart/GazeCapture/GazeCapture_data/GazeCapture_anno_gaze.txt'
    # data_df(datadir)
    ...
    # 計算每個bin的百分比
    hist_x, bins_x = np.histogram(data_x, bins, density=True)

    # 將密度轉換為百分比
    hist_x_percent = hist_x * np.diff(bins_x) * 100

    # 繪製直方圖
    plt.hist(data_x, bins=bins, weights=np.ones_like(data_x) / len(data_x) * 100, alpha=0.7, label='EVG')
